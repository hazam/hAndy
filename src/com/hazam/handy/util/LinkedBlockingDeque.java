...
public class LinkedBlockingDeque<E> extends AbstractQueue<E> implements BlockingQueue<E>, java.io.Serializable {
   ...
   * method differs from {@link #poll poll} only in that it throws an exception
   ...
   * such an element exists). Returns <tt>true</tt> if this deque contained the
   ...
   * 
   * <p>
   * Suppose <tt>x</tt> is a deque known to contain only strings. The following
   ...
   * 
   * @param a
   *          the array into which the elements of the deque are to be stored,
   ...
   * returned <tt>Iterator</tt> is a "weakly consistent" iterator that will
  ...
  private abstract class AbstractItr implements Iterator<E> {
     ...
     * exists in hasNext(), we must return item read under lock (in advance())
     * even if it was in the process of being removed when hasNext() was called.
     */
    E nextItem;
    /**
    ...
    /**
     * Advances next, or if not yet initialized, sets to first node. Implemented
     * to move forward vs backward in the two subclasses.
     */
